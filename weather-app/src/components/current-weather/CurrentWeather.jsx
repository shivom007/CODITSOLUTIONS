...
const CurrentWeather = ({data,loading}) => {
    if(!loading) return <></>
  return (
    <div className="flex flex-col justify-evenly md:justify-around md:p-[1.5rem] p-[0.5rem] bg-[#f8f8ff] border-gray-400 md:h-[18rem] h-[40vh] border-1 md:w-full w-[95vw]  max-w-[45rem] rounded-[1.5rem]">
      <h1 className="md:text-2xl text-xl text-gray-500">Current Weather</h1>
      <div className="flex items-center box-border md:flex-wrap flex-row">
        <div className="flex flex-col md:w-[25rem] w-[10rem]">
          <h4 className="md:text-3xl text-xl font-semibold text-[#396bae]">{data.name}</h4>
          <div className="flex items-center   justify-start">
            <img
              ...
              alt=""
            />
            <h1 className="md:text-8xl text-3xl  text-[#4a6fa1]">
              {Math.round(data.main.temp)}°C
            </h1>
          </div>
          <h6 className="md:text-2xl text-xl text-gray-400 font-bold">{data.weather[0].description}</h6>
        </div>
        <div className="flex flex-col text-[#7b98b2] w-auto md:gap-5">
          <p className="md:text-2xl text-base">
            Feels like <span className='text-[#3080c8] font-semibold'>

            {Math.round(data.main.feels_like)}°C
            </span>
          </p>
          <div className="flex items-center justify-start">
            <div className="flex  w-[8rem] items-center md:text-[1rem] text-base text-[#7b98b2]">
            ...
            <span className='font-semibold md:text-[1rem] text-[0.8rem] text-[#3080c8]'>{data.main.humidity}%</span>
          </div>
          <div className="flex items-center justify-start">
            <div className="flex w-[8rem] items-center md:text-[1rem] text-base text-[#7b98b2]">
            ...
            <span className='font-semibold md:text-[1rem] text-[0.8rem] text-[#3080c8]'>{data.wind.speed} m/s</span>
          </div>
          <div className="flex items-center justify-start">
            <div className='flex w-[8rem] items-center md:text-[1rem] text-base text-[#7b98b2]'>
            ...
            <span className='font-semibold md:text-[1rem] text-[0.8rem] text-[#3080c8]'>{data.main.pressure} hPa</span>
          </div>
        </div>
      </div>
    </div>
